...
//********************************************************************************
    ...
    std::function<float()> eventweight = [&]() { return wvz.looper_eventweight() * wvz.looper_lepsf() * wvz.looper_btagsf(); };

    ana.cutflow.addCut("ChannelEMu"     , [&]() { return wvz.ChannelEMu();     } , eventweight );
    ana.cutflow.addCut("ChannelOffZ"    , [&]() { return wvz.ChannelOffZ();    } , eventweight );
    ana.cutflow.addCut("ChannelOnZ"     , [&]() { return wvz.ChannelOnZ();     } , eventweight );
    ana.cutflow.addCut("ChannelBTagEMu" , [&]() { return wvz.ChannelBTagEMu(); } , eventweight );

    ana.cutflow.getCut("ChannelEMu");
    ana.cutflow.addCutToLastActiveCut("ChannelEMuHighMT", [&]() { return wvz.looper_lep3MT() > 40. and wvz.looper_lep4MT() > 20.; }, UNITY);

    ana.cutflow.getCut("ChannelOffZ");
    ana.cutflow.addCutToLastActiveCut("ChannelOffZHighMET", [&]() { return wvz.met_pt() > 100.; }, UNITY );

    // Print cut structure
    ana.cutflow.printCuts();

    // Histogram utility object that is used to define the histograms
    // I always use 180 bin because it's a highly composite number. so I can rebinned them in many different bins later easily.
    ana.histograms.addHistogram("looper_lep3MT", 180, 0, 250, [&]() { return wvz.looper_lep3MT(); } );
    ana.histograms.addHistogram("looper_lep4MT", 180, 0, 250, [&]() { return wvz.looper_lep4MT(); } );
    ana.histograms.addHistogram("met_pt", 180, 0, 300, [&]() { return wvz.met_pt(); } );
    ana.histograms.addHistogram("met_orig_pt", 180, 0, 300, [&]() { return wvz.met_orig_pt(); } );
    ...
    while (ana.looper.nextEvent())
    {
        ...
        // Event lists
        ana.cutflow.setEventID(wvz.run(), wvz.lumi(), wvz.evt());
    ...
    // Prefix of the output eventlist file path
    TString eventlist_output_file_name_prefix = ana.output_tfile->GetName();
    eventlist_output_file_name_prefix.ReplaceAll(".root", "_eventlist_");

    ana.cutflow.getCut("ChannelEMuHighMT").writeEventList(eventlist_output_file_name_prefix + "_ChannelEMuHighMT.txt");
    ana.cutflow.getCut("ChannelOffZHighMET").writeEventList(eventlist_output_file_name_prefix + "_ChannelOffZHighMET.txt");
